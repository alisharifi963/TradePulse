// دیگه نیازی به import ethers نیست، چون از CDN لود می‌شه
const { providers, Contract, utils } = window.ethers;

// RPCهای شبکه‌ها
const providersMap = {
  base: new providers.JsonRpcProvider("https://mainnet.base.org"),
  ethereum: new providers.JsonRpcProvider(`https://mainnet.infura.io/v3/${process.env.INFURA_API_KEY}`),
  arbitrum: new providers.JsonRpcProvider("https://arb1.arbitrum.io/rpc"),
...
const quoterAddresses = {
  uniswap: {
    ...
    arbitrum: "0x1F8c9623aD0C2e63cDBeC0EeBEd2BaF54d2dA3D", // آدرس درست برای Arbitrum
  },
  pancakeswap: {
...
async function getUniswapRate(network, tokenIn, tokenOut, amountIn, decimalsTo) {
  ...
  const provider = providersMap[network];
  const quoterContract = new Contract(quoterAddresses.uniswap[network], QUOTER_ABI, provider);
  const fee = 3000; // 0.3%
  try {
    const amountOut = await quoterContract.quoteExactInputSingle(tokenIn, tokenOut, fee, amountIn, 0);
    return utils.formatUnits(amountOut, decimalsTo);
  } catch (error) {
...
async function getPancakeSwapRate(network, tokenIn, tokenOut, amountIn, decimalsTo) {
  ...
  const provider = providersMap[network];
  const quoterContract = new Contract(quoterAddresses.pancakeswap[network], QUOTER_ABI, provider);
  const fee = 2500; // 0.25%
  try {
    const amountOut = await quoterContract.quoteExactInputSingle(tokenIn, tokenOut, fee, amountIn, 0);
    return utils.formatUnits(amountOut, decimalsTo);
...
async function getCurveRate(network, tokenIn, tokenOut, amountIn, decimalsTo) {
  ...
  const poolAddress =
    network === "ethereum"
      ...
      : "0x7f90122BF0700F9E7e1F688fe926940E8839F353"; // Pool واقعی برای Arbitrum
  const poolContract = new Contract(poolAddress, CURVE_POOL_ABI, provider);
  try {
